...
namespace gpgmp
{
	namespace mpfRoutines
	{
		...
		ANYCALLER void
		mpf_ui_sub(mpf_ptr r, unsigned long int u, mpf_srcptr v, mp_limb_t* scratchSpace)
		{
#if 1
			__mpf_struct uu;
			mp_limb_t ul;
			   ...
			   parts of) U and V, to simplify the code below.  */
				...
				if (usize > ediff)
				{
					...
					if (ediff == 0)
					{
						...
						if (1 < vsize)
						{
							...
							if (ulimb > vp[vsize])
							{
								...
								ASSERT_CARRY(gpgmp::mpnRoutines::gpmpn_neg(tp, vp, vsize));
							}
							else
							{
					...
					else
					{
						...
						ASSERT(vsize == 1 && usize >= 2 && ulimb == *vp);
						{
							...
							size = usize - ediff - 1;
							MPN_COPY(tp, up, size);
							ASSERT_NOCARRY(gpgmp::mpnRoutines::gpmpn_sub_1(tp + size, up + size, usize - size, ulimb));
							rsize = usize;
						}
				...
				else
				{
					...
					for (i = vsize; i < size; i++)
						tp[i] = GMP_NUMB_MAX;
					ASSERT_NOCARRY(gpgmp::mpnRoutines::gpmpn_sub_1(tp + size, up, usize, CNST_LIMB(1)));
				}
