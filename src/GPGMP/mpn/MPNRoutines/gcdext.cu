...
namespace gpgmp
{
  namespace mpnRoutines
  {
    ...
    */
    ANYCALLER static size_t hgcd_mul_matrix_vector(struct hgcd_matrix *M, mp_ptr rp, mp_srcptr ap, mp_ptr bp, mp_size_t n, mp_ptr tp)
    {
      mp_limb_t ah, bh;
...
#define COMPUTE_V_ITCH(n) (2 * (n))
    ...
    ANYCALLER static mp_size_t compute_v(mp_ptr vp, mp_srcptr ap, mp_srcptr bp, mp_size_t n, mp_srcptr gp, mp_size_t gn, mp_srcptr up, mp_size_t usize, mp_ptr tp)
    {
      mp_size_t size;
      mp_size_t an;
      mp_size_t bn;
...
#define CHOOSE_P_2(n) ((n) / 3)
    ...
    copies of a and b. */
        scratch = MPN_GCDEXT_LEHMER_N_ITCH(GCDEXT_DC_THRESHOLD) + 3 * GCDEXT_DC_THRESHOLD;
      ...
      if (an > n)
      {
        mp_limb_t *scratchForTDivQR = TMP_ALLOC_LIMBS(gpgmp::mpnRoutines::gpmpn_tdiv_qr_itch(an, n));
        gpmpn_tdiv_qr(tp, ap, 0, ap, an, bp, n, scratchForTDivQR);
