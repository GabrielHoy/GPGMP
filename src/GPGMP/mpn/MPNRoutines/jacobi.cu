...
#define BITS_FAIL 31
    ...
    ANYCALLER static void jacobi_hook(void *p, mp_srcptr gp, mp_size_t gn, mp_srcptr qp, mp_size_t qn, int d)
    {
      ...
      if (qp)
      {
        ASSERT(qn > 0);
        ASSERT(d >= 0);
        *bitsp = gpmpn_jacobi_update(*bitsp, d, qp[0] & 3);
      }
    }
...
#define CHOOSE_P(n) (2 * (n) / 3)
    ...
    HOSTONLY int gpmpn_jacobi_n(mp_ptr ap, mp_ptr bp, mp_size_t n, unsigned bits)
    {
      ...
      while (ABOVE_THRESHOLD(n, JACOBI_DC_THRESHOLD))
      {
        ...
        nn = gpmpn_hgcd_jacobi(ap + p, bp + p, n - p, &M, &bits,
                             tp + matrix_scratch);
        if (nn > 0)
        {
          ASSERT(M.n <= (n - p - 1) / 2);
        ...
        {
          /* Temporary storage n */
          n = gpmpn_gcd_subdiv_step(ap, bp, n, 0, jacobi_hook, &bits, tp);
          if (!n)
          {
            TMP_FREE;
            return bits == BITS_FAIL ? 0 : gpmpn_jacobi_finish(bits);
          }
        }
      }
      ...
      while (n > 2)
      {
        ...
        if (gpmpn_hgcd2_jacobi(ah, al, bh, bl, &M, &bits))
        {
      ...
      if (n == 1)
      {
        ...
        if (bl == 1)
          ...
          return gpmpn_jacobi_base(al, bl, bits << 1);
      }

      else
      {
        int res = gpmpn_jacobi_2(ap, bp, bits & 1);
        TMP_FREE;
        return res;
      }
    }
