/* gpmpn_toom32_mul -- Multiply {ap,an} and {bp,bn} where an is nominally 1.5
...
#define scratch_out (scratch + 2 * n + 1) /* Currently unused. */
      ...
      ap1_hi = gpmpn_add(ap1, a0, n, a2, s);
#if HAVE_NATIVE_gpmpn_add_n_sub_n
      if (ap1_hi == 0 && gpmpn_cmp(ap1, a1, n) < 0)
      {
        ap1_hi = gpmpn_add_n_sub_n(ap1, am1, a1, ap1, n) >> 1;
        hi = 0;
        vm1_neg = 1;
      }
      else
      {
        cy = gpmpn_add_n_sub_n(ap1, am1, ap1, a1, n);
        hi = ap1_hi - (cy & 1);
        ap1_hi += (cy >> 1);
        vm1_neg = 0;
      }
      ...
      if (ap1_hi == 0 && gpmpn_cmp(ap1, a1, n) < 0)
      {
        ASSERT_NOCARRY(gpmpn_sub_n(am1, a1, ap1, n));
        hi = 0;
        vm1_neg = 1;
      }
      else
      {
        hi = ap1_hi - gpmpn_sub_n(am1, ap1, a1, n);
        vm1_neg = 0;
      }
      ap1_hi += gpmpn_add_n(ap1, ap1, a1, n);
#endif
      ...
      if (t == n)
      {
        ...
        {
          cy = gpmpn_add_n_sub_n(bp1, bm1, b1, b0, n);
          vm1_neg ^= 1;
        }
        else
        {
          cy = gpmpn_add_n_sub_n(bp1, bm1, b0, b1, n);
        }
        bp1_hi = cy >> 1;
#else
        bp1_hi = gpmpn_add_n(bp1, b0, b1, n);

        if (gpmpn_cmp(b0, b1, n) < 0)
        {
          ASSERT_NOCARRY(gpmpn_sub_n(bm1, b1, b0, n));
          vm1_neg ^= 1;
        }
        else
        {
          ASSERT_NOCARRY(gpmpn_sub_n(bm1, b0, b1, n));
        }
#endif
      }
      else
      {
        /* FIXME: Should still use gpmpn_add_n_sub_n for the main part. */
        bp1_hi = gpmpn_add(bp1, b0, n, b1, t);

        if (gpmpn_zero_p(b0 + t, n - t) && gpmpn_cmp(b0, b1, t) < 0)
        {
          ASSERT_NOCARRY(gpmpn_sub_n(bm1, b1, b0, t));
          MPN_ZERO(bm1 + t, n - t);
          vm1_neg ^= 1;
        }
        else
        {
          ASSERT_NOCARRY(gpmpn_sub(bm1, b0, n, b1, t));
        }
      }
      ...
      if (ap1_hi == 1)
      {
        cy = gpmpn_add_n(v1 + n, v1 + n, bp1, n);
      }
      else if (ap1_hi > 1) /* ap1_hi == 2 */
      {
#if HAVE_NATIVE_gpmpn_addlsh1_n_ip1
        cy = gpmpn_addlsh1_n_ip1(v1 + n, bp1, n);
#else
        cy = gpmpn_addmul_1(v1 + n, bp1, n, CNST_LIMB(2));
#endif
      }
      else
        cy = 0;
      if (bp1_hi != 0)
        cy += ap1_hi + gpmpn_add_n(v1 + n, v1 + n, ap1, n);
      v1[2 * n] = cy;

      TOOM32_MUL_N_REC(vm1, am1, bm1, n, scratch_out);
      if (hi)
        hi = gpmpn_add_n(vm1 + n, vm1 + n, bm1, n);
      ...
      if (vm1_neg)
      {
        ...
        gpmpn_rsh1sub_n(v1, v1, vm1, 2 * n + 1);
#else
        gpmpn_sub_n(v1, v1, vm1, 2 * n + 1);
        ASSERT_NOCARRY(gpmpn_rshift(v1, v1, 2 * n + 1, 1));
#endif
      }
      else
      {
#if HAVE_NATIVE_gpmpn_rsh1add_n
        gpmpn_rsh1add_n(v1, v1, vm1, 2 * n + 1);
#else
        gpmpn_add_n(v1, v1, vm1, 2 * n + 1);
        ASSERT_NOCARRY(gpmpn_rshift(v1, v1, 2 * n + 1, 1));
#endif
      }
     ...
     --+----++----+----+-
      ...
      hi = vm1[2 * n];
      cy = gpmpn_add_n(pp + 2 * n, v1, v1 + n, n);
      ...
      if (vm1_neg)
      {
        cy = gpmpn_add_n(v1, v1, vm1, n);
        hi += gpmpn_add_nc(pp + 2 * n, pp + 2 * n, vm1 + n, n, cy);
        MPN_INCR_U(v1 + n, n + 1, hi);
      }
      else
      {
        cy = gpmpn_sub_n(v1, v1, vm1, n);
        hi += gpmpn_sub_nc(pp + 2 * n, pp + 2 * n, vm1 + n, n, cy);
        MPN_DECR_U(v1 + n, n + 1, hi);
      }

      TOOM32_MUL_N_REC(pp, a0, b0, n, scratch_out);
      /* vinf, s+t limbs.  Use gpmpn_mul for now, to handle unbalanced operands */
      if (s > t)
        gpmpn_mul(pp + 3 * n, a2, s, b1, t);
      else
        gpmpn_mul(pp + 3 * n, b1, t, a2, s);
      ...
      */
      ...
      cy = gpmpn_sub_n(pp + n, pp + n, pp + 3 * n, n);
      hi = scratch[2 * n] + cy;

      cy = gpmpn_sub_nc(pp + 2 * n, pp + 2 * n, pp, n, cy);
      hi -= gpmpn_sub_nc(pp + 3 * n, scratch + n, pp + n, n, cy);

      hi += gpmpn_add(pp + n, pp + n, 3 * n, scratch, n);

      /* FIXME: Is support for s + t == n needed? */
      if (LIKELY(s + t > n))
      {
        hi -= gpmpn_sub(pp + 2 * n, pp + 2 * n, 2 * n, pp + 4 * n, s + t - n);
