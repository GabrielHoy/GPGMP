...
#define TOOM4_SQR_REC(p, a, n, ws)                                        \
  ...
  {                                                                       \
    ...
    else if (MAYBE_sqr_toom2 && BELOW_THRESHOLD(n, SQR_TOOM3_THRESHOLD))  \
      gpmpn_toom2_sqr(p, a, n, ws);                                         \
    else if (!MAYBE_sqr_toom4 || BELOW_THRESHOLD(n, SQR_TOOM4_THRESHOLD)) \
...
#define amx (pp + 4 * n + 2) /* n+1 */
      ...
      gpmpn_toom_eval_dgr3_pm2(apx, amx, ap, n, s, tp);
      ...
      gpmpn_toom_eval_dgr3_pm1(apx, amx, ap, n, s, tp);
      ...
      gpmpn_toom_interpolate_7pts(pp, n, (enum toom7_flags)0, vm2, vm1, v2, vh, 2 * s, tp);
    }
