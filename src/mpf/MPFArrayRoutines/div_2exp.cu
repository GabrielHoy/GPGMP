...
namespace gpgmp
{
  namespace mpfArrayRoutines
  {
    ...
    ANYCALLER void
    gpmpf_div_2exp(mpf_array_idx r, mpf_array_idx u, mp_bitcnt_t exp)
    {
      mp_srcptr up;
      ...
      else
      {
        ...
        if (abs_usize > prec)
        {
          ...
          cy_limb = gpgmp::mpnRoutines::gpmpn_rshift(rp + 1, up, abs_usize, exp % GMP_NUMB_BITS);
          rp[0] = cy_limb;
          adj = rp[abs_usize] != 0;
        }
        else
        {
          cy_limb = gpgmp::mpnRoutines::gpmpn_lshift(rp, up, abs_usize,
        ...
        abs_usize += adj;
        MPF_ARRAY_EXPONENTS(r.array)[r.idx] = uexp - exp / GMP_NUMB_BITS - 1 + adj;
      }
