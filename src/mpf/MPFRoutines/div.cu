...
namespace gpgmp
{
  namespace mpfRoutines
  {
    ...
    /*ORIGINAL IMPLEMENTATION: CONTAINS DYNAMIC ALLOCATION
    ANYCALLER void
    gpmpf_div(mpf_ptr r, mpf_srcptr u, mpf_srcptr v)
    {
      mp_srcptr up, vp;
      mp_ptr rp, scratchSpace, new_vp;
      mp_size_t usize, vsize, rsize, prospective_rsize, tsize, zeros;
      mp_size_t sign_quotient, prec, high_zero, chop;
      mp_exp_t rexp;
      int copy_u;
      ...
      if (copy_u)
      {
        scratchSpace = TMP_ALLOC_LIMBS(tsize + 1); // +1 for mpn_div_q's scratch needs
        MPN_ZERO(scratchSpace, zeros);
        MPN_COPY(scratchSpace + zeros, up, usize);
        up = scratchSpace;
        usize = tsize;
      ...
      {
        scratchSpace = TMP_ALLOC_LIMBS(usize + 1);
      }
