...
namespace gpgmp {
    ...
    namespace host {
        ...
        //Returns true if all data assignment(s) and/or memory allocation(s) could be completed, false otherwise.
        HOSTONLY bool mpn_array_copy_to_mpz_array(mpz_t* arrayCopyInto, mpn_host_array& arrayCopyFrom) {
            ...
            for (int idxToCopy = 0; idxToCopy < arrayCopyFrom->numIntegersInArray; idxToCopy++) {
                ...
                mp_limb_t* copiedLimbData = reinterpret_cast<mp_limb_t*>(memcpy(limbDataToCopyInto, limbDataArray + (idxToCopy * arrayCopyFrom->numLimbsPerInteger), limbCountToCopy * sizeof(mp_limb_t)));
                if (copiedLimbData == NULL) {
                    return false;
                }
        ...
        //Assumes that the given mpz_t array has enough array indices to store all integers from the given mpn_array.
        //Returns true if all data assignment(s) and/or memory allocation(s) could be completed, false otherwise.
        HOSTONLY bool mpn_array_copy_to_mpz_array_with_mpz_init(mpz_t* arrayCopyInto, mpn_host_array& arrayCopyFrom) {
            mp_limb_t* limbDataArray = MPN_ARRAY_DATA(arrayCopyFrom);
            int* sizesArray = MPN_ARRAY_SIZES(arrayCopyFrom);
            ...
            for (int idxToCopy = 0; idxToCopy < arrayCopyFrom->numIntegersInArray; idxToCopy++) {
                mpz_t& copyInto = arrayCopyInto[idxToCopy];
                const int limbCountToCopy = sizesArray[idxToCopy];
                mpz_init2(copyInto, MPN_ARRAY_BITS_FROM_LIMB_COUNT(limbCountToCopy)); //Initializes the mpz_t with exactly enough precision to hold the limb data we need to copy from arrayCopyFrom.
                mp_limb_t* copiedLimbData = reinterpret_cast<mp_limb_t*>(memcpy(copyInto->_mp_d, limbDataArray + (idxToCopy * arrayCopyFrom->numLimbsPerInteger), limbCountToCopy * sizeof(mp_limb_t)));
                if (copiedLimbData == NULL) {
                    return false;
                }
                SIZ(copyInto) = limbCountToCopy;
        ...
        HOSTONLY cudaError_t mpn_array_copy_to_mpz_array_from_device(mpz_t* arrayCopyInto, mpn_device_array& arrayCopyFrom, const int arraySize, const mp_bitcnt_t precision) {
            ...
            int numLimbsPerInteger = MPN_ARRAY_LIMB_COUNT_FROM_BITS(precision);
        ...
        HOSTONLY cudaError_t mpn_array_copy_to_mpz_array_from_device_with_mpz_init(mpz_t* arrayCopyInto, mpn_device_array& arrayCopyFrom, const int arraySize, const mp_bitcnt_t precision) {
            ...
            for (int idxToCopy = 0; idxToCopy < arraySize; idxToCopy++) {
                ...
                mpz_init2(copyInto, MPN_ARRAY_BITS_FROM_LIMB_COUNT(ABS(limbCountToCopy))); //Initializes the mpz_t with exactly enough precision to hold the limb data we need to copy from arrayCopyFrom.
