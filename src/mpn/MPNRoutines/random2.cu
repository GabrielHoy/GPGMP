/* gpmpn_random2 -- Generate random numbers with relatively long strings
...
namespace gpgmp
{
  namespace mpnRoutines
  {
    ...
    ANYCALLER static void gmp_rrandomb(mp_ptr, gmp_randstate_t, mp_bitcnt_t);
...
#define BITS_PER_RANDCALL 32
    ...
    ANYCALLER void
    gpmpn_random2(mp_ptr rp, mp_size_t n)
    {
    ...
    gmp_rrandomb(mp_ptr rp, gmp_randstate_t rstate, mp_bitcnt_t nbits)
    {
      ...
      for (;;)
      {
        ...
        gpmpn_incr_u(rp + bi / GMP_NUMB_BITS, CNST_LIMB(1) << bi % GMP_NUMB_BITS);
