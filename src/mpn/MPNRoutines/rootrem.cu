...
#define LOGROOT_RETURNED_BITS (LOGROOT_USED_BITS + LOGROOT_NEEDS_TWO_CORRECTIONS)
    /* Puts in *rootp some bits of the k^nt root of the number
       ...
       one is not counted).
     */
    ANYCALLER static unsigned logbased_root(mp_ptr rootp, mp_limb_t op, mp_bitcnt_t bitn, mp_limb_t k)
    {
      /* vlog=vector(256,i,floor((log(256+i)/log(2)-8)*256)-(i>255)) */
      static const unsigned char vlog[] = {1, 2, 4, 5, 7, 8, 9, 11, 12, 14, 15, 16, 18, 19, 21, 22,
      ...
      static const unsigned char vexp[] = {0, 1, 2, 2, 3, 4, 4, 5, 6, 7, 7, 8, 9, 9, 10, 11,
      ...
      if (UNLIKELY(bitn > (~(mp_bitcnt_t)0) >> LOGROOT_USED_BITS))
      {
        ...
        bitn = (bitn << LOGROOT_USED_BITS |
                vlog[op >> (GMP_NUMB_BITS - LOGROOT_USED_BITS)]) /
               k;
      }
      else
      {
      ...
      *rootp = CNST_LIMB(1) << (LOGROOT_USED_BITS - !LOGROOT_NEEDS_TWO_CORRECTIONS) | vexp[bitn] >> !LOGROOT_NEEDS_TWO_CORRECTIONS;
      return retval;
    }
