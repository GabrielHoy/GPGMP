...
#define getbit(p, bi) \
  ((p[(bi - 1) / GMP_NUMB_BITS] >> (bi - 1) % GMP_NUMB_BITS) & 1)
    ...
    ANYCALLER static inline mp_limb_t getbits(const mp_limb_t *p, mp_bitcnt_t bi, int nbits)
    {
      int nbits_in_r;
      mp_limb_t r;
      mp_size_t i;
...
#define POWM_SEC_TABLE 2, 130, 524, 2578
    ...
    ANYCALLER extern int win_size(mp_bitcnt_t);
#else
    ANYCALLER static inline int win_size(mp_bitcnt_t enb)
    {
      int k;
    ...
    ANYCALLER static void redcify(mp_ptr rp, mp_srcptr up, mp_size_t un, mp_srcptr mp, mp_size_t n, mp_ptr tp)
    {
      MPN_ZERO(tp, n);
      MPN_COPY(tp + n, up, un);
    ...
    ANYCALLER static mp_limb_t sec_binvert_limb(mp_limb_t n)
    {
      mp_limb_t inv, t;
      ASSERT((n & 1) == 1);
      /* 3 + 2 -> 5 */
