...
#define DO_mpn_addlsh_n(dst, src, n, s, ws) mpn_addlsh_n(dst, dst, src, n, s)
#else
    ANYCALLER static mp_limb_t DO_mpn_addlsh_n(mp_ptr dst, mp_srcptr src, mp_size_t n, unsigned int s, mp_ptr ws)
    {
#if USE_MUL_1 && 0
    ...
    ANYCALLER int mpn_toom_eval_pm2rexp(mp_ptr rp, mp_ptr rm, unsigned int q, mp_srcptr ap, mp_size_t n, mp_size_t t, unsigned int s, mp_ptr ws)
    {
      unsigned int i;
      int neg;
